...
describe('Deck', () => {
    it('Market should fill up normally', done => {
        ...
        deck.market.cards.length.should.equal(0)
        deck.fillMarket()

        deck.market.cards.length.should.be.oneOf(
    ...
    it('Market should fill up all the way because a set can\'t be made', done => {
        ...
        deck.market.cards.should.be.empty()
        deck.fillMarket()
        deck.market.cards.length.should.be.equal(CardsWithoutSet.length)
    ...
    it('Should remove cards_ and keep order', done => {
        ...
        deck.market.cards.length.should.eql(8)

        const removedCards = deck.market.popSet(1, 3, 6)
        ...
        deck.market.cards.length.should.eql(5)
        for(const card of deck.market.cards)
